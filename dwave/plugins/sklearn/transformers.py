...
class HybridFeatureSelection(BaseEstimator, SelectorMixin):
    
    def __init__(self, alpha : float = .5, time_limit : int = 10, n_default_feature : int = 10, method = "correlation") -> None:
        """
        ...
        self.mask = None
        assert (self.alpha <= 1) and (self.alpha >= 0), f"alpha {self.alpha} is not between 0 and 1"
        assert self.time_limit >1, "Time limit must be positive and greater than 1"
        
        self.acceptable_methods = ["correlation", "mutual information"]
        
        assert method in self.acceptable_methods, f"method was {method}, must be one of {self.acceptable_methods}"
    ...
    def _get_support_mask(self):
        ...
        support : boolean array of shape [# input features]
            ...
            retention.
        """
        assert self.mask is not None, "fit hasn't been run yet"
        
        return self.mask
    
    def calculate_correlation_matrix(self, X, y = None , chunksize = 500):
        """_summary_
        ...
        """
        logging.info("Starting correlation calculation") # heavy logging to diagnose memory performance 
        ...
        if (X.shape[1] < chunksize):
            ...
            with tempfile.NamedTemporaryFile() as temp:
                correlation_matrix = np.memmap(temp, 'float64', mode='w+', shape=(n_features, n_features))
            ...
            for row_index in range(0, n_features, chunksize):
                for col_index in range(0, n_features, chunksize):
                    ...
                    correlation_matrix[row_index:row_index_max, col_index:col_index_max] = chunk1.T @ chunk2
        ...
        if y is not None:
            ...
            np.fill_diagonal(correlation_matrix, self.alpha*(1 / y.shape[0])*(X_standardized.T @ y_standardized))
        else:
    ...
    def fit(self, X, y=None, number_of_features = None, strict : bool = True):
        """
        ...
        if self.method == "correlation":
            ...
            assert y is not None, "mutual infromation requires outcome"
            model_matrix = self.calculate_mutual_information(X,y)
        else: 
            raise NotImplementedError(f"Only methods {self.acceptable_methods} are implimented")
        ...
        if (X.shape[1] < chunksize):
            ...
            feature_selection_bqm.add_variables_from({i: model_matrix[i,i] for i in range(model_matrix.shape[0])}) 
            ...
            for row_index in range(0, model_matrix.shape[0], chunksize):
                for col_index in range(0, model_matrix.shape[1], chunksize):
                        ...
                        chunk = model_matrix[row_index:row_index_max, col_index:col_index_max]
        ...
        feature_selection_cqm.add_constraint_from_iterable([(var, 1) for var in feature_selection_cqm.variables], 
                                                        sense, 
                                                        number_of_features)
        ...
        assert self.time_limit > min_time_limit, f"the time limit must be at least for this problem {min_time_limit}"
        ...
        selected_features = [index for index,val in feature_sample_best.sample.items() if val == 1]
        ...
        if isinstance(X, pd.DataFrame): 
            ...
            self.mask = np.array([(i in self.selected_columns) for i in range(X.shape[1])])
    ...
    def update_time_limit(self, time_limit) -> None:
        """
